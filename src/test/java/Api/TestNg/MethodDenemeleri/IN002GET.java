...
public class IN002GET {
    ...
    @Test(priority = 1, description = "Geçerli token ile kursları listeleme testi")
    ...
    @Test(priority = 2, description = "Token olmadan yapılan istekte 401 hatası kontrolü")
    public void testGetCoursesWithoutToken() {
        // 4. Yetkisiz Erişim Testi
        given()
                .header("User-Agent", USER_AGENT)
                .header("x-api-key", X_API_KEY)
                .header("Accept", "application/json")
                // Authorization header'ı bilerek eklenmedi
                .when()
                .get("/api/courses")
                .then()
                .statusCode(401);

        System.out.println("✅ Token olmadan yapılan istekte beklenen 401 hatası alındı.");
    }

    @Test
    public void tc003(){

        API_Methods.sendRequest("GET","courses");
